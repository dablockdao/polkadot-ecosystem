...
* **SubWallet (extension + mobile).** Broad Polkadot SDK/EVM coverage; solid Ledger integration (Generic app).
...
### Block explorers
* **Subscan.** Widely used Polkadot SDK explorer suite with deep chain data, events/extrinsics breakdowns, and APIs.
...
| SubWallet      | Extension, Mobile     | Yes                       | Ledger (Generic app)      | Polkadot SDK + EVM | Broad coverage; desktop + mobile        |
| Polkadot Vault | iOS/Android (offline) | Via QR with connected UI  | Air-gapped signer         | Polkadot SDK family         | High-security signing for teams         |

### Explorers (comparison)
| Explorer   | Scope                        | Open source | Strengths                                | Typical use                          |
| ---------- | ---------------------------- | ----------- | ---------------------------------------- | ------------------------------------ |
| Subscan    | Polkadot SDK ecosystem          | Partial     | Deep chain data, events/extrinsics, APIs | Daily driver for accounts/extrinsics |
...
**Subscan** for deep Polkadot SDK data; **Statescan** for Asset Hub/system chains; **3xpl** for quick multi-chain checks; **XCSCAN** for cross-chain routes.
