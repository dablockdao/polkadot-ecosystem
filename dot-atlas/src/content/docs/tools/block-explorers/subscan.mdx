...
sidebar:
  ...
  order: 1
hero:
  tagline: BWeb3 Portal to the Polkadot SDK (formerly Substrate) & EVM Ecosystems
  image: 
...
**Subscan** is a one-stop, multi-chain **block explorer** for the Polkadot SDK ecosystem (Polkadot, Kusama, Asset Hub, and parachains) with growing EVM support. It positions itself as “Explorer as a Service” (EaaS), meaning the same productized explorer runs across many networks with consistent UX and endpoints.
...
For teams and integrators, Subscan offers a **hosted API** that serves indexed data for dozens of networks across the Polkadot SDK ecosystem.
...
At a high level Subscan runs indexers that follow each chain’s blocks, parsing **extrinsics/events** (and EVM data where present). That data powers both the web explorer and dedicated **HTTP APIs**. There’s also **Subscan Essentials**, an open-source scaffold for Polkadot SDK explorers that underpins subscan.io deployments—handy to study architecture or self-host a minimal explorer.

Subscan complements developer-centric tools listed in the official Polkadot docs (e.g., Blockscout/Routescan for EVM/PolkaVM contexts) and sits alongside other Polkadot SDK explorers in the broader tooling landscape.
...
| ------------------------------- | ------------------------------------- | ---------------------------------------------------- | ------------------------------ | ------------------------------------------------------ | --------------------- | --------------------------- | -------------------------------------------------------------------------------- |
| **Subscan**                     | Extensive Polkadot SDK + many parachains | **Yes** (XCM dashboards: flows, messages, transfers) | **Yes** (referenda v2 details) | **Yes** (EVM input decode, ERC-20 price, ERC-1155 qty) | **Yes** (hosted API)  | **Essentials** scaffold OSS | Portfolio, watchlists, quick staking actions.                                    |
...
| **Polkaholic**                  | High parachain coverage               | **Strong** (XCM monitoring)                          | Some governance linkages       | Polkadot SDK-first                                        | Public endpoints vary | Parts open                  | Focus on cross-chain account & XCM analytics.                                    |
| **Polkadot.js Apps (Explorer)** | Any connected Polkadot SDK chain         | No XCM visuals                                       | Minimal governance UI          | N/A                                                    | N/A                   | **Yes**                     | Best for dev-grade **RPC** interactions; Subscan offers friendlier read-only UX. |
...
* **Scope & coverage.** Subscan’s hosted HTTP API exposes indexed data for many Polkadot SDK networks (chain-agnostic endpoints like `block`, `extrinsic`, `account`, plus network-specific).
