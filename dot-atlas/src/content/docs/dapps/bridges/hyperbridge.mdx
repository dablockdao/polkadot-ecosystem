...
* Result: cross-chain actions that are **auditable, permissionless, and general-purpose**—across EVMs, Polkadot SDK (formerly Substrate) chains, and more.
...
2. **Register the counterparty**
   Configure the target chain’s identifier, client type, and verification parameters in your chain’s config. Choose the **consensus client** appropriate for the counterparty (e.g., Ethereum, Polkadot SDK-based, rollup).
