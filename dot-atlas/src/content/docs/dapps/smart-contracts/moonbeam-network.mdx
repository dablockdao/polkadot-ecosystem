...
Moonbeam is fully EVM-compatible and uses **unified accounts**: your standard Ethereum H160 address (0x…) controls both EVM and Polkadot SDK (formerly Substrate)-based features (staking, governance, XCM) on Moonbeam. This removes the “two-accounts” friction found on many EVM-on-Polkadot SDK chains.
...
**XC-20** = ERC-20 that’s also XCM-aware. Any ERC-20 deployed on Moonbeam can be configured as an XC-20 and sent to/from parachains using XCM. Moonbeam exposes **X-Tokens/XCM precompiles** so Solidity contracts can call Polkadot SDK XCM methods directly (addresses are stable across networks).
...
> Think of MRL as a **port parachain**: liquidity from Ethereum/Solana/etc. arrives via a GMP protocol, then Moonbeam forwards it natively to your Polkadot destination (e.g., Hydration (formerly HydraDX), Interlay) in one flow.
...
**Goal:** End with USDC (or a target representation) on a destination parachain (e.g., **Hydration**) without manual hops.

1. **Choose origin & destination**
   * Origin: Ethereum (USDC).
   * Destination: Target parachain needing liquidity (e.g., Hydration).
...
5. **Route (XCM step)**
   * On delivery, Moonbeam forwards the payload via XCM to the parachain (e.g., Hydration), crediting your address there.
