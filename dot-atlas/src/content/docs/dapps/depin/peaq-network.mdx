...
* **ink!**: Rust-based smart contracts (Polkadot SDK (formerly Substrate) native), useful when you need fine-grained control and Wasm-first features.
...
These functions are exposed via SDKs so web2/web3 teams can integrate quickly without deep Polkadot SDK expertise.

## Developer experience: SDKs, tooling, runtimes
* **SDKs:** Official **JavaScript** SDK, with **Python** also available.
* **Explorers & infra:** EVM/Polkadot SDK explorers and managed node services are available for development and production.
