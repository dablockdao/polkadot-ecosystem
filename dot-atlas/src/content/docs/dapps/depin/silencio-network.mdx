...
Silencio runs on **peaq**, a Polkadot ecosystem chain that acquires blockspace on Polkadot via Coretime (active parathread status). That gives Silencio access to Polkadot SDK (formerly Substrate) tooling and Polkadot’s scalability roadmap while keeping DePIN-specific functions close to the protocol layer.
