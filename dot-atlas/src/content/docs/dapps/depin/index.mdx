...
### Energy Web X
**What it is:** An energy-sector DePIN building a public Polkadot SDK (formerly Substrate) chain centered on worker node networks that run energy workflows and coordinate assets, with a Polkadot integration strategy.
...
**Polkadot SDK L1 mentions:**
* **CESS** — Polkadot SDK L1 for persistent storage plus a decentralized delivery layer. Consider when you need frequent reads and latency-sensitive UX.
...
| Energy Web X   | Energy worker node networks     | Public Polkadot SDK network           | Workflows on worker nodes            | Meters, EVSE, grid assets        | Certificates, grid services, mobility energy |
...
> Notes: Some projects operate as parachains today. Others integrate through the Polkadot SDK and Polkadot tooling while parachain status or timelines evolve. Confirm mainnet details in project docs before deployment.
...
<DirectoryLinks directory="dapps/depin" />