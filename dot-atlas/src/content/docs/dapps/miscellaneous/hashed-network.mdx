...
hero:
  tagline: Blockchain for businesses.
  image: 
...
## Hashed Network General Architecture and Components
...
1. **Afloat Client API:** Provides methods to interact with the gated Marketplace, Uniques, and Fruniques pallets, facilitating the Afloat-specific flow.
2. **Marketplace UI:** The user interface for the Hashed Network Portal, allowing users to interact with all system components.
3. **Confidential Documents Server and API:** Handles backend services for confidential documents, enabling secure storage and sharing of sensitive information.
4. **Faucet Server:** Provides token distribution services for new accounts, reducing user onboarding friction.
5. **Bitcoin Orchestration:** Recent Bitcoin innovations, such as BIP 174 and the Taproot upgrade, enable developers to decouple the signing experience from transaction orchestration. Hashed Network leverages these innovations for Bitcoin orchestration.
...
## Hashed Network Community Videos
------------
<ShowcaseYouTube
  entries={[
    {
      href: 'https://www.youtube.com/watch?v=Y34yi-kdxJM',
      title: 'Hashed Network Connecting Solochain to Polkadot:',
      description: 'ashed Network and the $1B secured on their solochain ready to connect with Polkadot.'    },
  ]} />