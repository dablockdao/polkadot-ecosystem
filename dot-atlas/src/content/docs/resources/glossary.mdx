...
Beyond balances, accounts can hold locks (staking, democracy, vesting), identity data, and proxy delegations (e.g., proxy for governance only). On Polkadot SDK-based chains, an account’s existential deposit (ED) prevents dust accounts; dropping below ED “reaps” the account.
...
### API
For Polkadot, primary access is via JSON-RPC. **PAPI (Polkadot-API)** is the recommended modern TypeScript client with light-client-first architecture and code-generated types. **Dedot** offers a lightweight, tree-shakable alternative. **Subxt** provides a Rust-based client. The mature **polkadot-js API** remains a stable option for legacy projects. The metadata endpoint drives type-safe interactions across all clients; keep it in sync after runtime upgrades.
...
Pipelined parachain block production where collators can produce the next block candidate without waiting for the current one to be included on the relay chain. This decouples collation from relay-chain block times, reducing latency and increasing throughput. [Learn more →](/resources/concepts/asynchronous-backing)
...
### Availability Cores
Relay-chain execution/availability slots assigned to parachains/threads under Agile Coretime.
...
In the Polkadot SDK, the state root commits to all storage; light clients can verify inclusion proofs without downloading full state.
...
### Bridge Hub
System parachain providing trust-minimized connections to external networks via light-client bridges. Used by Snowbridge (Ethereum bridge) and other cross-ecosystem routes. [Learn more →](/resources/concepts/bridge-hub)

### Bulletin Chain
Polkadot’s decentralized, content-addressed storage layer for Product data. Supports authorization-based writes, chunked uploads, and automatic renewal. IPFS-compatible content addressing. [Learn more →](/resources/concepts/bulletin-chain)
...
On Polkadot SDK-based chains, you can target multiple execution models: smart-contracts pallets, runtime pallets (native), or off-chain workers. Choose based on upgrade cadence and trust minimization.

Always test flows on Paseo/Rococo with faucets before mainnet.
...
### Dedot
Lightweight, tree-shakable TypeScript client for Polkadot with first-class support for Polkadot-API (PAPI) typed interactions. Generates minimal bundle sizes through tree-shaking.

### dNS (dotNS)
Decentralized naming system on Polkadot, analogous to ENS on Ethereum. Maps human-readable names to addresses, with a .dot namespace and marketplace for name trading. [Learn more →](/resources/concepts/dotns)
...
### Elastic Scaling
Ability for a single parachain to use multiple availability cores simultaneously, dynamically scaling its blockspace based on demand. Reduces latency and increases throughput during high-traffic periods. [Learn more →](/resources/concepts/elastic-scaling)
...
### FRAME
Polkadot SDK’s pallet library for building runtimes.
...
### Fork
Polkadot SDK-based chains upgrade by replacing the Wasm runtime on-chain, avoiding social-layer hard forks for most changes. Runtime versions signal compatibility to wallets and SDKs.
...
### HOP
Highway Overlay Protocol: a network relay framework that enables permissionless, verifiable message routing across Polkadot system chains. Provides efficient message delivery with cryptographic proofs. [Learn more →](/resources/concepts/hop)
...
### Inclusion Pipeline
The multi-stage process by which a parachain candidate progresses from collation to finalization on Polkadot. Covers backing, availability, and approval stages with defined timeouts and fallback mechanisms. [Learn more →](/resources/concepts/inclusion-pipeline)
...
### ink!
Rust-based smart contract language for Polkadot SDK chains, compiling to Wasm. Offers type-safe contract development with a built-in test framework and a standard library (openbrush) for common patterns.
...
### JAM
Join-Accumulate Machine: A next-generation protocol design that evolves Polkadot into a global, permissionless supercomputer. Combines elements of the relay chain with smart-contract execution in a unified, code-first model. [Learn more →](/resources/concepts/jam)
...
Polkadot SDK-based chains use tries to commit to storage; proofs allow light clients to verify values.
Storage migrations must preserve keys/paths or provide migration logic; otherwise proofs break.

### Moonwall
Developer testing framework for Polkadot SDK chains, providing local devnet orchestration, automated test scenarios, and integration testing against running nodes.
...
On Polkadot SDK-based chains, multisig is an on-chain approval workflow; add/remove signers requires a new multisig.
...
### Omni Node
Full-node RPC aggregator that provides a unified endpoint for querying Polkadot relay chain and parachains. Eliminates the need to run separate nodes for each chain. [Learn more →](/tools/dev/omninode)
...
### PAPI (Polkadot-API)
Modern TypeScript client library for Polkadot with a light-client-first architecture. Generates type-safe, tree-shakeable API bindings from metadata. Recommended as the primary client for new TypeScript projects, replacing the older polkadot-js API approach.

### Paseo
Community-managed test network for parachain development. Serves as the primary testing ground after the deprecation of Westend and Rococo. Features regular runtime upgrades and a dedicated faucet.

### People Chain
System parachain managing on-chain identity, proof-of-personhood verification, and social-recovery mechanics. Provides a unified identity layer across all Polkadot system chains. [Learn more →](/resources/concepts/people-chain)

### PolkaVM
RISC-V-based virtual machine for smart contract execution on Polkadot SDK chains. Supports multiple source languages (Rust, C, C++) via LLVM compilation and offers deterministic execution with formal verification paths.

### Polkadot Hub
Central system parachain bridging the relay chain to end users. Provides account abstraction, balance management, and cross-chain orchestration. The primary entry point for user interactions on Polkadot. [Learn more →](/resources/concepts/polkadot-hub)

### Polkadot Products
The suite of Polkadot-native applications and services including the Polkadot Hub, Asset Hub, Bridge Hub, People Chain, and Bulletin Chain. Provides a complete ecosystem for identity, assets, storage, and bridging. [Learn more →](/resources/concepts/polkadot-products)

### Pop CLI
Command-line tool for scaffolding, building, and deploying Polkadot SDK chains and ink! smart contracts. Streamlines the development workflow with templates, testing utilities, and deployment scripts.

### Proof of Personhood
Sybil-resistance mechanism using identity vouching and web-of-trust to establish unique human identity on-chain. Implemented via the People Chain with Ring-VRF zero-knowledge proofs for privacy. [Learn more →](/resources/concepts/proof-of-personhood)

### Pallet
Polkadot SDK runtime module implementing domain logic.
...
### Parity Technologies
Core developer of the Polkadot SDK and Polkadot implementations.
...
### REVM
Rust Ethereum Virtual Machine implementation optimized for Polkadot SDK chains. Enables EVM-compatible smart contract execution with full Solidity support and high performance.

### Revive
EVM-to-Polkadot SDK transpiler that converts Solidity smart contracts to ink! contracts. Enables Ethereum developers to deploy existing Solidity code on Polkadot SDK chains without manual rewrites.

### Root Origin
Highest-privilege origin in the Polkadot SDK.
...
### SDK
Tooling for building apps and chains (e.g., Polkadot SDK).

### Snowbridge
Trust-minimized bridge between Polkadot and Ethereum, using light-client verification for both chains. Enables permissionless token and message transfer without intermediary validators. [Learn more →](https://docs.snowbridge.network)

### Statement Store
Decentralized, content-addressed data availability layer built on the Bulletin Chain. Stores arbitrary data blobs with author-based access control and automatic content-addressed retrieval. [Learn more →](/resources/concepts/statement-store)

### Subxt
Rust client library for submitting transactions and querying storage on Polkadot SDK chains. Generates type-safe interfaces from runtime metadata, offering compile-time checks for on-chain interactions.
...
Dev loop: write pallet → add benchmarks → integrate in runtime → test on Paseo → ship via OpenGov.
...
On Polkadot SDK-based chains, transfers check ED (existential deposit) and optional keep-alive. Batched or XCM transfers can fail partially; design idempotent retries and surface refunds.
