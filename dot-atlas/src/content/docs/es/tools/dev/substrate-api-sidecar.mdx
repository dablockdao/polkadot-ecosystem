---
title: 'Sidecar'
description: 'Interactúa fácilmente con nodos de blockchain basados en Polkadot SDK usando Substrate API Sidecar, un potente servicio REST para desarrolladores.'
hero:
...
El **Substrate API Sidecar** es un servicio RESTful diseñado para facilitar la interacción fluida con nodos de blockchain construidos con el framework Polkadot SDK (FRAME). Con una amplia variedad de endpoints, la API de Sidecar otorga a los desarrolladores la capacidad de consultar y administrar datos de blockchains basadas en Polkadot SDK con gran precisión y eficiencia. Ya sea para interactuar con cuentas, bloques, transacciones, parachains u otros componentes de la blockchain, la API de Sidecar es una herramienta esencial para los desarrolladores que trabajan dentro de los ecosistemas de **Polkadot** y **Polkadot SDK** —y más allá.

## ¿Qué es Substrate API Sidecar?
Substrate API Sidecar funciona como una interfaz REST para interactuar con nodos Polkadot SDK. Simplifica las interacciones complejas con la blockchain, permitiendo a los desarrolladores:
...
Gracias a su facilidad de uso, se ha convertido en un recurso indispensable para quienes crean herramientas y aplicaciones en los ecosistemas de **Polkadot** y **Polkadot SDK**.
...
3. **Despliegue Personalizable.** De manera predeterminada, el servicio Sidecar se conecta a un nodo Polkadot SDK local, pero puede configurarse para interactuar con nodos remotos o distintos entornos mediante parámetros ajustables, como `SUBSTRATE_URL`.
...
## Inicia el Servicio y Conéctalo a un Nodo Polkadot SDK
Inicia el servicio y conéctalo a un nodo Polkadot SDK en ejecución. Por defecto, el servicio se ejecuta en `http://127.0.0.1:8080`.
