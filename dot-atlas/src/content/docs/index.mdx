...
Polkadot is a **network of blockchains**. A central **relay chain** provides shared security; specialized **parachains** plug in to add capabilities (DeFi, identity, gaming, etc.). The **Polkadot Hub** system chain serves as the primary user entry point, providing account abstraction, balance management, and cross-chain orchestration. **XCM** is the native messaging standard for moving assets and instructions across chains. **Asset Hub** is a system chain for issuing and managing tokens/NFTs that other chains can use; some networks support **paying fees in non-native assets** via asset-conversion pools. **Dual-VM smart contracts** (ink! Wasm + EVM/Solidity) allow developers to deploy on Polkadot SDK chains using their preferred language.

## How the ecosystem is organized
* **Relay Chain:** Security & coordination; not for dapps.
* **System Chains:**
  * **Polkadot Hub** for account abstraction, balance management, and cross-chain orchestration.
  * **Asset Hub** for assets/NFTs and fee-payment features via conversion pools.
  * **People Chain** for on-chain identity, proof-of-personhood, and social recovery.
...
entries={[
      ...
      }, 
 {
      ...
      }, 
    {
      ...
      description: 'Find smart-contract ecosystems built on Polkadot and Polkadot SDK.'
      },
    {
...
* **Polkadot Docs (concepts):** Clear explanations for tracks, convictions, time-locks, and delegation approaches.
...
**Smart contracts**
* **ink! (Wasm):** Rust-native smart contracts on Polkadot SDK-based chains.
* **EVM / PolkaVM paths:** Deploy Solidity using Foundry-Polkadot or Hardhat integrations; or target EVM-compatible parachains.

**APIs & clients**
* **PAPI (Polkadot-API):** Modern TypeScript client with light-client-first architecture, code-generated types, and tree-shakeable bundles. Recommended for new projects.
* **polkadot-js API (TS):** Mature, stable option for querying chain state and submitting extrinsics; suitable for legacy projects.
* **Dedot:** Lightweight, tree-shakable TypeScript client with first-class PAPI support.
* **Subxt:** Rust client with compile-time type safety from metadata.
* **RPC & indexing:**
...
### Gaming
* **Mythos / Ajuna / Astar / Moonbeam gaming ecosystems:** Mix of EVM and Polkadot SDK tooling for in-game assets, marketplaces, and auth.
...
| ---------- | ---------------------------- | ----------- | ---------------------------------------- | ------------------------------------ |
| Subscan    | Polkadot ecosystem          | Partial     | Deep chain data, events/extrinsics, APIs | Daily driver for accounts/extrinsics |
...
-------------
<CardGrid>
  <Card title="Polkadot Docs" icon="open-book">
    Explore the official Polkadot technical documentation—guides, references, and deep dives across the entire protocol.
    <LinkButton href="https://docs.polkadot.com" variant="secondary" icon="external" iconPlacement="start">Polkadot Docs</LinkButton>
  </Card>
